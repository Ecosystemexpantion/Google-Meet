...
(async function () {
  ...
  function tick() {
    ...
    if (diff <= 0) { clearInterval(cdInterval); refreshUI(); return; }
  ...
  function handleJoin() {
    ...
    const payload = { id: participantId, name: firstName, fullName: raw, sessionId: activeSession.id, roomName: activeSession.room_name };
    sessionStorage.setItem('eem26_p', JSON.stringify(payload));
    const params = new URLSearchParams({ n: firstName, pid: participantId, sid: activeSession.id, rm: activeSession.room_name });
