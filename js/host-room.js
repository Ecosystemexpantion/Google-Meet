...
(async function () {
  ...
  let jitsiApi       = null;
  let participants   = new Map(); // id → row

  // ── Week-based room name ──────────────────────────────────
  ...
  function buildRoomName() {
    const now = new Date();
    const w   = getWeek(now);
    return `${CFG.ROOM_PREFIX}${now.getFullYear()}W${String(w).padStart(2, '0')}`;
  }
  ...
  async function endSession() {
    ...
    const ok = confirm('End the session now? All students will see the closing message.');
    if (!ok) return;
    ...
    await sb.from('participants')
      .update({ has_speaking_permission: false, is_active: false, left_at: new Date().toISOString() })
    ...
    if (jitsiApi) {
      try { jitsiApi.executeCommand('endConference'); } catch (_) {}
    }
  ...
  async function initPresence() {
    if (!currentSession) return;
    const ch = sb.channel(`presence:${currentSession.room_name}`, {
      config: { presence: { key: 'host' } },
    });
    ch.on('presence', { event: 'sync' }, () => {
      const count = Object.values(ch.presenceState()).flat().length;
      watchingEl.textContent = count;
    });
    await ch.subscribe(async (status) => {
      if (status === 'SUBSCRIBED') await ch.track({ name: 'Host' });
    });
  }
  ...
  function renderParticipants() {
    ...
    const handRaisedCount = list.filter(p => p.has_raised_hand).length;
    handsEl.textContent = handRaisedCount;
    ...
    // Sort: hand-raised first, then by join time
    list.sort((a, b) => {
  ...
  // ── Grant / Revoke permission (exposed globally for onclick) ──
  window.hostGrantPermission = async (participantId) => {
    await sb.from('participants')
      .update({ has_speaking_permission: true, has_raised_hand: false })
      .eq('id', participantId);
    const p = participants.get(participantId);
    if (p) { p.has_speaking_permission = true; p.has_raised_hand = false; }
    renderParticipants();
  };

  window.hostRevokePermission = async (participantId) => {
    await sb.from('participants')
      .update({ has_speaking_permission: false })
      .eq('id', participantId);
    const p = participants.get(participantId);
    if (p) p.has_speaking_permission = false;
    renderParticipants();
  };
  ...
  muteAllBtn.addEventListener('click', async () => {
    if (!currentSession) return;
    // Revoke all speaking permissions in Supabase (triggers student browsers)
    await sb.from('participants')
      .update({ has_speaking_permission: false })
      .eq('session_id', currentSession.id);
    // Also mute all via Jitsi moderator control (belt-and-suspenders)
    if (jitsiApi) {
      try { jitsiApi.executeCommand('muteEveryone', 'audio'); } catch(_) {}
    }
  });
  ...
  if (existingSession) {
    ...
    if (existingSession.is_active) {
      ...
      subscribeParticipants();
      await initPresence();
      initJitsi(existingSession.room_name);
    }
  } else {
    updateSessionUI(false);
  }
